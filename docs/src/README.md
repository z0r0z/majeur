...
Moloch.sol has been scanned by thirty-three independent audit tools. Reports with per-finding review notes are in [`/audit`](./audit/). Formal verification specs and harnesses are in [`/certora`](./certora/).
...
| [Nemesis](./audit/nemesis.md) | [Iterative deep-logic audit (Feynman + State Inconsistency)](https://github.com/0xiehnnkta/nemesis-auditor) | 6 (0C, 0H, 1M, 4L, 1I — 1 novel informational, 5 duplicates) | Full coupled-state invariant verification across all 5 contracts. 0 state desync bugs. 1 novel cosmetic observation (NEM-05: non-minting sale seats DAO in badge system). All other findings are duplicates of KF#1, KF#3, KF#11, KF#15, KF#16. No production blockers |
| [Plamen](./audit/plamen.md) | [Autonomous multi-agent audit (4 opus depth agents, plamen methodology)](https://github.com/PlamenTSV/plamen) | 2 Low, 3 Info (0 novel) | All 8 key invariants verified, 12 assembly blocks verified clean, 10 vulnerability categories covered. L-01 (auto-futarchy overcommitment) is a variant of KF#3's soft accounting. L-02 (arbitrary ID opens) extends KF#21 with sharper root cause — existing fix insufficient but bounded by SafeSummoner. No production blockers |

**No production blockers were identified across any audit.** Fourteen novel Moloch.sol core findings were surfaced across thirty-three scans (5 from prior audits + 5 from Cantina covering peripheral contracts and namespace issues + 2 from Archethect V2 + 1 from Auron + 1 from Nemesis). Winfunc added 0 novel Moloch.sol core findings but identified 12 novel root causes in peripheral contracts (LPSeedSwapHook, ShareBurner, TapVest, ShareSale, Tribute, MolochViewHelper) — candidates for extraction into per-contract audit folders under `src/peripheral/audit/`. Cantina additionally identified ~18 novel frontend findings (XSS and logic bugs) — the first audit to cover the dapp. Configuration-dependent concerns are enforced by [`SafeSummoner`](./src/peripheral/SafeSummoner.sol); code-level issues are candidates for v2 hardening.

**Novel smart contract findings (14):**
...
14. Non-minting sale seats DAO contract address in badge system — `buyShares` non-minting path calls `shares.transfer` from DAO, triggering `onSharesChanged(DAO)` which can seat the DAO contract in the top-256 badge bitmap, consuming a slot. Cosmetic only — governance-recoverable via `burnSeat`, SafeSummoner defaults to `minting = true` (Nemesis — Informational, novel cosmetic observation)
...
- **Nemesis** is the first audit to perform exhaustive coupled-state invariant verification — systematically mapping every state pair (7 groups across 5 contracts) and verifying every mutation path for desync. The Feynman pass (deep-logic questioning) produced 6 findings, all duplicates except NEM-05 (non-minting sale seats DAO contract in badge system — informational, cosmetic). The State Inconsistency pass confirmed all invariants hold: ERC-6909 mint/burn symmetry, vote tally/receipt coupling, Shares checkpoint consistency, Badges bitmap/seat/ERC-721 coherence, and futarchy resolution timing. Converged in 2 passes (no new findings from cross-feed). Signal-to-noise: 1 novel from 6 total (17%), but the novel finding is informational. The primary value is the structural verification that no state desync bugs exist.

- **Plamen** is the first audit using the autonomous multi-agent plamen methodology — 4 parallel opus-model depth agents (token-flow, state-trace, adversarial reasoning, edge-case/external) with full SECURITY.md cross-referencing against all 24 known findings. All 8 key invariants verified, 12 assembly blocks verified clean, all 10 vulnerability categories covered. L-01 (concurrent auto-futarchy overcommitment for local reward tokens) sharpens KF#3's soft-accounting surface for the specific `address(shares)`/`address(loot)` configuration path. L-02 (arbitrary ID opens) extends KF#21 with a broader root cause analysis — the KF#21 permit-only fix is insufficient, but SafeSummoner's `proposalThreshold > 0` enforcement and `autoFutarchyCap` bound the impact. Signal-to-noise: 0 novel from 5 total (0%), but the KF#21 root cause sharpening is analytically valuable.

Cross-referencing across all thirty-three scans — fourteen independent novel Moloch.sol core findings (plus ~18 novel frontend findings from Cantina and 12 novel peripheral root causes from Winfunc), twenty-four catalogued known findings (KF#1–24), consistent duplicate confirmation across tools, 142 formally verified invariants, and full coupled-state invariant verification (Nemesis) — increases confidence that the known findings represent the full Moloch.sol core attack surface. Cantina's coverage of the frontend and peripheral contracts (Tribute, DAICO) opened a new surface area not previously audited. Archethect V2's ragequit front-run and transfer lock bypass findings extend coverage to economic timing attacks and access control edge cases not previously explored. Winfunc's peripheral coverage (LPSeedSwapHook, ShareBurner, TapVest, ShareSale, Tribute, MolochViewHelper) identified 12 novel root causes in contracts not systematically audited before — these findings are candidates for extraction into per-contract security documents under `src/peripheral/audit/`. Nemesis's state inconsistency analysis provides structural confidence that no coupled-state desync bugs exist across any mutation path.
...
DAOs deployed through `SafeSummoner.safeSummon()` cannot hit the configuration footguns identified across the thirty-three audits. The `previewCalls()` function lets frontends inspect exactly which `initCalls` will execute, and `predictDAO()` returns the deterministic address before deployment. An `extraCalls` escape hatch preserves full flexibility for advanced setups (custom allowances, etc.).
...
*Reviewed by thirty-three auditing tools (see [Audits](#audits)) — no formal manual audit. Use at your own risk.*
