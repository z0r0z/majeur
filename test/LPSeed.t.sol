...
import {LPSeed} from "../src/peripheral/LPSeed.sol";
...
contract LPSeedTest is Test {
    SafeSummoner internal safe;
    LPSeed internal lpSeed;
    ...
    /// @dev Deploy a DAO with LPSeed configured for ETH + shares pair.
    function _deployWithSeed(
        ...
        extra[3] = Call(
            ...
            abi.encodeCall(
                lpSeed.configure,
                (address(0), amtA, sharesAddr, amtB, 30, 100, deadline, shareSaleAddr, minSupply)
            )
        );
    ...
    /// @dev Deploy a DAO with ShareSale + LPSeed (sale completion gate).
    function _deployWithSaleAndSeed(
        ...
        // 6 initCalls: ShareSale setup (2) + mint shares for seed (1) + LPSeed setup (3)
        ...
        extra[1] = Call(
            address(shareSale), 0, abi.encodeCall(shareSale.configure, (dao, address(0), 1e18))
        );

        // Mint shares to DAO for LP seeding
        extra[2] = Call(sharesAddr, 0, abi.encodeCall(Shares.mintFromMoloch, (dao, seedAmtB)));

        // LPSeed: setAllowance(ETH) + setAllowance(shares) + configure
        extra[3] = Call(
    ...
    function test_RevertIf_ConfigureZeroAmounts() public {
        vm.prank(alice);
        vm.expectRevert(LPSeed.InvalidParams.selector);
        lpSeed.configure(address(0), 0, address(1), 100, 30, 100, 0, address(0), 0);
    }

    function test_RevertIf_ConfigureZeroTokenB() public {
        vm.prank(alice);
        vm.expectRevert(LPSeed.InvalidParams.selector);
        lpSeed.configure(address(0), 100, address(0), 100, 30, 100, 0, address(0), 0);
    }

    function test_RevertIf_ConfigureSameTokens() public {
        vm.prank(alice);
        vm.expectRevert(LPSeed.InvalidParams.selector);
        lpSeed.configure(address(1), 100, address(1), 100, 30, 100, 0, address(0), 0);
    }

    function test_DefaultSlippage() public {
        vm.prank(alice);
        lpSeed.configure(address(0), 100, address(1), 100, 30, 0, 0, address(0), 0);

        (,,,,, uint16 maxSlipBps,,,,) = lpSeed.seeds(alice);
        assertEq(maxSlipBps, 100); // defaults to 1%
    }
    ...
    function test_RevertIf_SeedBeforeDeadline() public {
        ...
        vm.expectRevert(LPSeed.NotReady.selector);
        lpSeed.seed(dao);
    }
    ...
    function test_RevertIf_CancelNotConfigured() public {
        ...
        vm.expectRevert(LPSeed.NotConfigured.selector);
        lpSeed.cancel();
    }
    ...
    function test_Seedable_CombinedDeadlineAndSale() public {
        ...
        extra[5] = Call(
            ...
            abi.encodeCall(
                lpSeed.configure,
                (address(0), 1e18, sharesAddr, 100e18, 30, 100, deadline, address(shareSale), 0)
            )
        );

        // Need a 7th slot but we only have 6 calls, pad with a no-op
        // Actually we have exactly 6, trim the array
        Call[] memory trimmed = new Call[](6);
        for (uint256 i; i < 6; i++) {
            trimmed[i] = extra[i];
        }
        ...
        safe.safeSummon("ComboDAO", "COMBO", "", 1000, true, address(0), salt, h, s, c, trimmed);
    ...
    function test_Seed_ERC20Pair() public {
        ...
        extra[3] = Call(
            ...
            abi.encodeCall(
                lpSeed.configure,
                (address(usdc), usdcAmt, sharesAddr, sharesAmt, 30, 500, 0, address(0), 0)
            )
        );
        ...
        vm.expectRevert(LPSeed.AlreadySeeded.selector);
        lpSeed.seed(dao);
    }
    ...
    function test_SeedInitCallsHelper() public view {
        ...
        (address t1, bytes memory d1, address t2, bytes memory d2, address t3, bytes memory d3) = lpSeed.seedInitCalls(
            dao, address(0), 1e18, address(1), 1000e18, 30, 100, 0, address(0), 0
        );
