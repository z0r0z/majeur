...
> **Review: Duplicate of KF#16.** SECURITY.md KF#16 explicitly covers this: "`spendPermit` doesn't check `executed` flag — allows double-execution if DAO creates both proposal and permit with identical params. Requires two governance votes. `_burn6909` is the actual replay guard." The report's framing of `count > 1` enabling N-fold execution mischaracterizes the design: `count > 1` is intentional — permits with `count=5` are designed to be independently spendable by each receipt holder. Each `spendPermit` call burns exactly 1 receipt token, so each holder can only spend once. The `_burn6909` is the replay guard, not `executed`. The `executed[tokenId] = true` write exists to block the *governance path* (`executeByVotes`) from also executing the same intent — it is not dead, it cross-guards a different function. First identified by Zellic (#12), confirmed by Claude (Opus 4.6), catalogued as KF#16. **Severity: Low (by design, per KF#16).**
